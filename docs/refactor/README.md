...
- **The login server owns the account format.** Hash formats change only together with StarLoco-Login
...
The portal runs from `StarLoco-Game/docker-compose.yml` (service `starloco_web`), with `StarLoco-Web/`
bind-mounted at `/var/www/starloco-web` and `public/` served at `http://127.0.0.1/dofus/`. PHP and
template edits are live; CSS needs a rebuild.

```bash
cd StarLoco-Game
docker compose build starloco_web starloco_web_assets    # only the web images (see Phase 1 note on starloco_login)
docker compose up -d starloco_web                        # runs starloco_web_migrate and starloco_web_assets first
docker compose run --rm starloco_web_assets              # rebuild public/assets/app.css after editing templates/CSS
docker compose run --rm starloco_web_migrate             # apply new migrations
docker compose logs -f starloco_web                      # PHP errors
docker compose run --rm starloco_web_tools              # composer check: cs, PHPStan, Rector, all tests
```
