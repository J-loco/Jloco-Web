...
- [x] **Integration tests**, `tests/Integration`: repositories, `ShopService` (missing points,
      ...
      `jloco_web` DB user. Remember-me rotation is not covered yet.
      They create throwaway `jloco_login_test` / `jloco_game_test` databases from
...
- [x] **Experience table sync**: `bin/sync-experience [--check] [path/to/Experience.lua]` regenerates
      `src/Game/Experience.php`. The CI job runs only when the `JLOCO_GAME_TOKEN` secret (read-only token for
      the private JLoco-Game repository) is set. Otherwise it prints a skip message.
- [x] **Password hashing migration**, portal and JLoco-Login. Details below.
...
- [x] **Item images**: `bin/item-sprites` lists the (type, skin) pairs sold in the shop.
      `jloco_web_sprites` renders them from the client SWFs (JPEXS FFDec + ImageMagick) to
...
- [x] **Shop servers**: `SHOP_SERVERS="1:jloco_game,2:other_game_db"` maps `website_shop_objects.server` to
      the game database that receives the gift. `bin/migrate` grants `gifts` write access in each of them.

## Password hashing

Two formats are understood by both the portal (`Security\PasswordHasher`) and JLoco-Login
...
3. Do not go back to an old login server image (`jloco/login:latest` from Docker Hub): it cannot
...
## JLoco-Login changes made for this phase
...
- JUnit 4 tests (`test/`, `gradle test`), a multi-stage `Dockerfile` (tests run during the build) and
  `.github/workflows/ci.yml`. Compose now builds `jloco/login:local` from `../JLoco-Login`.
...
- Production ini ordering: `php-production.ini` was loaded before `zz-jloco.ini`, so
  `opcache.validate_timestamps` stayed on. The file was renamed to `zzz-jloco-production.ini`.

## How to run

From `JLoco-Game/` (vendor lives in the image, integration tests use the compose MariaDB):

```bash
docker compose build jloco_web_tools
docker compose run --rm jloco_web_tools                 # composer check: everything CI runs
docker compose run --rm jloco_web_tools test:unit       # or test, test:integration, stan, cs, cs:fix, rector, rector:fix
docker compose run --rm jloco_web_tools experience:check

docker compose run --rm jloco_web_tools item-sprites    # writes var/item-sprites.tsv
docker compose build jloco_web_sprites
docker compose run --rm jloco_web_sprites               # renders missing item images (--force: all)

docker compose --profile mail up -d jloco_mailpit       # catches reset emails at http://127.0.0.1:8025
# with WEB_MAILER_DSN=smtp://jloco_mailpit:1025 and WEB_MAIL_FROM=noreply@jloco.local in .env

docker compose build jloco_login                        # runs the Java tests
```
